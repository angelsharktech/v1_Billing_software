...
const SupplierLedger = () => {
  ...
  const handleExportClick = (event, client, payments) => {
    setSelectedSupplier({ client, payments });
    setAnchorEl(event.currentTarget);
  };
  ...
  const handleExport = (type) => {
    ...
    const { client, payments } = selectedSupplier;
    const exportData = payments.map((p) => ({
      date: p.date ? moment(p.date).format("DD/MM/YYYY") : "--",
      narration: p.narration || "--",
      dr: (p?.advanceAmount ?? 0).toString(),
      cr: (p?.balance ?? 0).toString(),
      closingAmount: (p?.closingAmount ?? 0).toString(),
    }));
...
${mainUser?.organization_id?.name || "Our Company"}`;
  ...
  const MobileSupplierCard = ({ client, payments }) => {
              ...
              <Stack spacing={1.5}>
                {payments.map((p, index) => (
                  <Box key={p._id} sx={{ 
                    ...
                    borderRadius: 1,
                    borderLeft: 3,
                    borderColor: p.balance > 0 ? 'warning.light' : 'success.light'
                  }}>
    ...
    <>
      <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }}>
        ...
        {isMobile ? (
          ...
          <Box>
            {filteredSuppliers.map(({ client, payments }) => (
              <MobileSupplierCard key={client._id} client={client} payments={payments} />
            ))}
        ...
        >
          ...
          {selectedSupplier && (
            <MenuItem onClick={() => handleExportPrint(selectedSupplier.client, selectedSupplier.payments)}>
              Print Ledger
            </MenuItem>
          )}
        </Menu>
