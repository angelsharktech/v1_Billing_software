...
const PurchaseBillForm = ({
  ...
  const calculateOverallTotals = () => {
    ...
    const taxableAmount = subtotal; // Taxable amount is only product subtotal (excluding additional charges for GST)

    // IMPORTANT: Additional charges are NOT included in GST calculation
    // They are added directly to grand total
  ...
  const handleVendorSelection = (value, type) => {
    ...
    if (type === "phone") {
      ...
      selectedVendor = users.find(
        (u) => u.phone_number === value && u.role_id?.name?.toLowerCase() === "vendor"
      );
    } else if (type === "name") {
      selectedVendor = users.find(
    ...
    try {
      ...
      if (!isExistingVendor) {
        ...
        const payload = {
          ...
          password: vendor.name.replace(/\s+/g, "").toLowerCase() + "@example.com",
      ...
      // Add shipping and packaging charges (NOT included in GST)
      const computedAdditionalCharges = Number(shippingCharge || 0) + Number(packagingCharge || 0);
      const computedTaxableAmount = computedSubtotal; // Only products are taxable
      ...
      if (res?.success === true || res?.data) {
        ...
        const paymentPayload = {
          ...
          closingAmount: Number((Number(finalVendor.openingAmount) + Number(finalTotals.grandTotal)).toFixed(2)),
        };
        ...
        if (advanceAmount > 0) {
          await addPayment({
            ...
            closingAmount: Number((paymentResult.data?.closingAmount - advanceAmount).toFixed(2)),
          });
        }

        // Update vendor's opening amount
        await updateUser(finalVendor._id, {
          openingAmount: Number((Number(finalVendor.openingAmount) + (Number(finalTotals.grandTotal) - Number(advanceAmount))).toFixed(2)),
        });

        // Reset form
        setVendor({ _id: "", name: "", address: "", phone_number: "", pincode: "", openingAmount: 0 });
        setSelectedProducts([{ productName: "", hsnCode: "", productCode: "", qty: 1, price: 0, gstPercent: 0, discountPercentage: "", discountedPrice: 0, isExisting: false, category: "", total: 0 }]);
        setAdvanceAmount(0);
        setShippingCharge(0);
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              onChange={(e) => setVendor({ ...vendor, openingAmount: Number(e.target.value) || 0 })}
              size="small"
              InputProps={{
