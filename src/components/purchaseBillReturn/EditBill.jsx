...
const EditBill = ({ open, data, handleCloseEdit, refresh }) => {
    ...
    const fetchBill = async () => {
      try {
        ...
        if (billData.balancePayMode.toLowerCase().includes("finance")) {
          ...
          setAdvance(advanceVal.toFixed(2));
        ...
        setSnackbarMessage("Error loading bill by ID", err);
  ...
  const handleAdvanceChange = async (e) => {
    ...
    setAdvance(Number(newAdvance.toFixed(2))); // Store just the new advance amount
    ...
    try {
      ...
      if (bill?.balancePayMode?.toLowerCase().includes("finance")) {
        ...
        updatedData = {
          ...
          balance: 0,
          paymentType: "full",
          fullPaid: Number(billTotal.toFixed(2)),
        };
        paymentType = "full";
        ...
        updatedData = {
          ...
          fullPaid: Number(isFullPayment ? billTotal.toFixed(2) : 0),
        };
      ...
      if (res.success === true) {
        ...
        let paymentPayload = {
          ...
          // amount: paymentType === "advance" ? advance : bill?.grandTotal || 0,
                    amount: Number(advance || 0).toFixed(2),
  ...
  return (
    <>
      ...
      >
        <Alert
          severity={
            snackbarMessage === "Purchase bill Updated !" ? "success" : "error"
          }
          onClose={() => setSnackbarOpen(false)}
          variant="filled"
        >
